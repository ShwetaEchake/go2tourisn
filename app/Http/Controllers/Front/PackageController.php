...
class PackageController extends Controller
{
    public function index()
    {
        ...
        $packages = DB::table('packages')->orderBy('p_seq','asc')->get();
    ...
    public function detail($slug)
    {
        ...
        $package_details = DB::table('packages')->where('services_id', $package_detail->services_id)->where('id', '!=', $package_detail->id)->where('destination_id', $package_detail->destination_id )->get();
        if(!$package_detail) {
            return abort(404);
        }
        return view('pages.package_detail', compact('g_setting','package_detail','package_photos','package_videos', 'package_details'));
    }
